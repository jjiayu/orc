...
from orc.optimal_control.lipm.biped.lipm_to_tsid import (
    compute_5th_order_poly_traj,
    generate_square_foot_trajectory,
)
...
# Disable meshcat ground grid
if hasattr(tsid_biped, "viz") and tsid_biped.viz is not None:
    tsid_biped.viz.viewer["/Grid"].set_property("visible", False)
...
if loaded_footsteps is not None:
    ...
    print(
        f"  Initial stance: {loaded_footsteps[0]['foot']} foot at [{loaded_footsteps[0]['position'][0]:.3f}, {loaded_footsteps[0]['position'][1]:.3f}, {loaded_footsteps[0]['position'][2]:.3f}] (not used as target)"
    )
    print("  Stepping targets:")
    for i, step in enumerate(actual_footsteps[:4]):  # Show first 4 actual targets
        pos = step["position"]
        print(
            f"    Target {i}: {step['foot']} foot -> [{pos[0]:.3f}, {pos[1]:.3f}, {pos[2]:.3f}], yaw: {step['yaw']:.3f}"
        )
...
def visualize_footsteps_in_meshcat(tsid_biped, footstep_targets, loaded_footsteps=None):
    ...
    # Footstep dimensions
    foot_length = 0.2  # x dimension
    foot_width = 0.1  # y dimension
        ...
        try:
            ...
            for group_idx, surface_group in enumerate(scene):
                ...
                for surface_idx, surface in enumerate(surface_group):
                    ...
                    center_z = (
                        z_min + z_max
                    ) / 2 - height / 2  # Shift down so top reaches z=0
                    ...
                    print(
                        f"  Surface {surface_count}: [{center_x:.2f}, {center_y:.2f}, {center_z:.2f}] size: [{width:.2f}x{depth:.2f}x{height:.2f}]"
                    )
                    surface_count += 1
...
load_and_visualize_environment(tsid_biped, "allterrain")
...
def visualize_goal(tsid_biped, footstep_targets):
    ...
    if (
        hasattr(tsid_biped, "viz")
        and tsid_biped.viz is not None
        and len(footstep_targets) > 0
    ):
        ...
        print(
            f"Added goal visualization at [{goal_pos[0]:.2f}, {goal_pos[1]:.2f}, {goal_pos[2]:.2f}]"
        )
    else:
        print(
            "Meshcat visualizer not available or no footsteps, skipping goal visualization"
        )
...
initial_com_height = calculate_com_height_for_phase(
    current_lf_pos, current_rf_pos, "double"
)
current_com[2] = initial_com_height

print(f"CoM Height Offset: {CoM_Height_Offset:.3f}m")
print(
    f"Initial CoM: [{current_com[0]:.3f}, {current_com[1]:.3f}, {current_com[2]:.3f}] (dynamic height)"
)
print(
    f"Initial LF:  [{current_lf_pos[0]:.3f}, {current_lf_pos[1]:.3f}, {current_lf_pos[2]:.3f}]"
)
print(
    f"Initial RF:  [{current_rf_pos[0]:.3f}, {current_rf_pos[1]:.3f}, {current_rf_pos[2]:.3f}]"
)
...
for phase_idx, contact_phase in enumerate(gait_pattern):
    ...
    phase_in_step = (
        phase_idx % 3
    )  # Which phase within the step (0=double, 1=stance, 2=double)
    ...
    if phase_in_step == 0:  # First double support phase of a step
        ...
        if step_number < len(footstep_targets):
            ...
            com_target[2] = calculate_com_height_for_phase(
                lf_start, rf_start, stance_foot
            )
        else:
        ...
        if step_number < len(footstep_targets):
            ...
            if swing_foot == "right":  # Right foot swings, left foot supports
                ...
                if loaded_footsteps is not None and step_number + 1 < len(
                    loaded_footsteps
                ):
                    rf_yaw_target = loaded_footsteps[step_number + 1][
                        "yaw"
                    ]  # +1 because we skipped first
                else:
                    rf_yaw_target = rf_yaw
                ...
                if loaded_footsteps is not None and step_number + 1 < len(
                    ...
                    lf_yaw_target = loaded_footsteps[step_number + 1][
                        "yaw"
                    ]  # +1 because we skipped first
                else:
                    lf_yaw_target = lf_yaw
    ...
    print(
        f"  Phase CoM height: {phase_com_height:.3f} (based on stance foot at phase start)"
    )
    print(
        f"  CoM: [{com_start[0]:.3f}, {com_start[1]:.3f}, {com_start[2]:.3f}] -> [{com_target[0]:.3f}, {com_target[1]:.3f}, {com_target[2]:.3f}]"
    )
    print(
        f"  LF:  [{lf_start[0]:.3f}, {lf_start[1]:.3f}, {lf_start[2]:.3f}] -> [{lf_target[0]:.3f}, {lf_target[1]:.3f}, {lf_target[2]:.3f}] | yaw: {np.degrees(lf_yaw_start):.1f}° -> {np.degrees(lf_yaw_target):.1f}°"
    )
    print(
        f"  RF:  [{rf_start[0]:.3f}, {rf_start[1]:.3f}, {rf_start[2]:.3f}] -> [{rf_target[0]:.3f}, {rf_target[1]:.3f}, {rf_target[2]:.3f}] | yaw: {np.degrees(rf_yaw_start):.1f}° -> {np.degrees(rf_yaw_target):.1f}°"
    )
    print(
        f"  Base yaw: {np.degrees(base_yaw_start):.1f}° -> {np.degrees(base_yaw_target):.1f}°"
    )
...
print(
    f"Total duration: {total_duration:.1f}s, N: {N}, N_pre: {N_pre}, N_post: {N_post}"
)
...
for i in range(-N_pre, N + N_post):
    ...
    if i == 0:
        ...
        if contact_pattern[i] != contact_pattern[i - 1]:
            print(
                f"Time {t:.3f}: Changing contact from {contact_pattern[i - 1]} to {contact_pattern[i]}"
            )
    ...
    if i < 0:
        ...
        tsid_biped.set_LF_6d_ref(
            current_lf_pos, np.zeros(3), np.zeros(3), initial_lf_yaw, 0.0, 0.0
        )
        tsid_biped.set_RF_6d_ref(
            current_rf_pos, np.zeros(3), np.zeros(3), initial_rf_yaw, 0.0, 0.0
        )
        ...
        tsid_biped.set_com_ref(
            com_pos_traj[:, i], com_vel_traj[:, i], com_acc_traj[:, i]
        )
        tsid_biped.set_LF_6d_ref(
            lf_pos_traj[:, i],
            lf_vel_traj[:, i],
        ...
        tsid_biped.set_LF_6d_ref(
            lf_pos_traj[:, -1], np.zeros(3), np.zeros(3), lf_yaw_traj[-1], 0.0, 0.0
        )
        tsid_biped.set_RF_6d_ref(
            rf_pos_traj[:, -1], np.zeros(3), np.zeros(3), rf_yaw_traj[-1], 0.0, 0.0
        )
    ...
    if i % conf.PRINT_N == 0:
        ...
        if i >= 0 and i < N:
            ...
            print(
                f"  Contact pattern: {contact_pattern[i]} | LF: {lf_contact} | RF: {rf_contact}"
            )
            ...
            print(
                f"  CoM target:  [{com_target[0]:.3f}, {com_target[1]:.3f}, {com_target[2]:.3f}]"
            )
            print(
                f"  CoM actual:  [{com_actual[0]:.3f}, {com_actual[1]:.3f}, {com_actual[2]:.3f}]"
            )
            print(
                f"  CoM tracking error: {norm(tsid_biped.comTask.position_error, 2):.3f}"
            )
            ...
            print(
                f"  LF target:   [{lf_target[0]:.3f}, {lf_target[1]:.3f}, {lf_target[2]:.3f}] | yaw: {np.degrees(lf_yaw_target):.1f}°"
            )
            print(
                f"  LF actual:   [{lf_actual[0]:.3f}, {lf_actual[1]:.3f}, {lf_actual[2]:.3f}] | yaw: {np.degrees(lf_yaw_actual):.1f}°"
            )
            print(
                f"  RF target:   [{rf_target[0]:.3f}, {rf_target[1]:.3f}, {rf_target[2]:.3f}] | yaw: {np.degrees(rf_yaw_target):.1f}°"
            )
            print(
                f"  RF actual:   [{rf_actual[0]:.3f}, {rf_actual[1]:.3f}, {rf_actual[2]:.3f}] | yaw: {np.degrees(rf_yaw_actual):.1f}°"
            )
            print(
                f"  Base target: {np.degrees(base_yaw_target):.1f}° | Base actual: {np.degrees(base_yaw_actual):.1f}°"
            )
...
metadata = {
    ...
    "joint_names": [
        tsid_biped.model.names[i] for i in range(2, len(tsid_biped.model.names))
    ],  # Skip universe and root
